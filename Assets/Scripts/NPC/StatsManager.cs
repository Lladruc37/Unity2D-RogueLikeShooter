...
public class StatsManager : NPCHealth
{
	...
	protected override void Awake()
	{
		if (npc.npcType == NPCType.Enemy)
		{
			var coef = 0.35f * PluginController.Instance.GetFeatureCoefficient("CHLGS")
				- 0.1f * PluginController.Instance.GetFeatureCoefficient("STATUS")
				+ 0.25f * PluginController.Instance.GetFeatureCoefficient("EXPLR")
				+ 0.30f * PluginController.Instance.GetFeatureCoefficient("LV&PROG");
			int addedHealthPercentage = (int)MathF.Ceiling(maxHealth * coef);
			maxHealth += addedHealthPercentage;
		}
